...
public class AuctionSniperTest {
	...
	public void report_won_when_auction_closed_when_any_state() {
		...
		context.checking(new Expectations(){{
			ignoring(auction);
			allowing(sniperListener).sniperStateChanged(new SniperSnapshot(ANY_ITEM_ID, 678, 768));
				then(sniperState.is(AuctionStatus.BIDDING.toString()));
			allowing(sniperListener).sniperWinning();
				then(sniperState.is(AuctionStatus.WINNING.toString()));
			allowing(sniperListener).sniperStateChanged(new SniperSnapshot(ANY_ITEM_ID, 778, 788));
				then(sniperState.is(AuctionStatus.BIDDING.toString()));
			allowing(sniperListener).sniperWinning();
				then(sniperState.is(AuctionStatus.WINNING.toString()));
